...
const Investment = ({ userEmail: propUserEmail }) => {
  ...
  const fetchUserPackage = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`https://v2-b.onrender.com/api/user/package?email=${encodeURIComponent(userEmail)}`);
  ...
  const handleUpgrade = async (packageType) => {
    ...
    try {
      ...
      const response = await fetch("https://v2-b.onrender.com/api/user/upgrade-package", {
        method: "PUT",
        headers: {
